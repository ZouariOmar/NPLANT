...
void InitSDL() {
  ...
  Window = SDL_CreateWindow(PROJECT_NAME, SDL_WINDOWPOS_CENTERED, SDL_WINDOWPOS_CENTERED, W, H, 0);
  ...
  if (!(Render))
    SDL_Log("> Erreur : Initialisation Renderer > %s\n", SDL_GetError()), SDL_DestroyWindow(Window), exit(EXIT_FAILURE);
}
...
/**
 * @brief ### Initialize the text color
...
/**
 * @brief ### Initialize the in-text screen position and format dimensions
 * @param pos {SDL_Rect *}
 * @param x {int}
 * @param y {int}
 * @param w {int}
...
/**
 * @brief ### Initialize the err-text font
 * @param Error {Text *}
 */
void InitError(Text* Error) {
  Error->Font = TTF_OpenFont("../../project/GUI/font/ARIAL.TTF", 30);
  if (!(Error->Font)) {
  ...
  strcpy(Error->Txt, "Error : 28 character max!");
...
void InitBackgroundimages(BG* Background) {
  Background->BG_Unselect = IMG_LoadTexture(Render, "../../project/GUI/Assets/Background.png");
  Background->BG_Select = IMG_LoadTexture(Render, "../../project/GUI/Assets/Background link selected.png");
}
